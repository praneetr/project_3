This application is intended to serve a stock market event historical run-up calculator. 

For example, Amazon company last reported earnings on February 1st, 2018. We can select the Amazon ticker (AMZN) and select the date of earnings (2/1/2018) and when we hit "Get Ticker Data" we will retrieve one month of historical stock prices for the selected ticker along with a % change comparison with the selected date which in this example is the earnings date. We can then see if the Amazon stock price was increasing leading up to earnings (which may signify an earnings beat). Note that the earnings date is the market event. This application can be used to lookback at any historical event (i.e. earnings, FDA trial announcements, merges or acquistions, etc...) and give us indications if there was a sell-off or run-up leading up to the event. 
...
Click the trashcan icon to delete the ticker data from mongoDB and have the ticker removed from the search history. 

Note that on intial page load you may have to refresh the screen to update the the search history. 


NOTE: Effective April 2018 QUANDL API is no longer providing free stock data. This application should be used for historical dates prior to 3/28/2018. QUANDL does provide paid options for real time data. 
